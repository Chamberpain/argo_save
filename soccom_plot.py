...
interpolated_pressure_list = range(0,1605,5)
...
def property_plot(df_):
	date_list = (df_.Date-df_.Date.min()).dt.days.values
	...
	pressure_list = df_.Pressure.values
	X,Y=np.meshgrid(date_list,interpolated_pressure_list) 
	Z = griddata(np.vstack((date_list,pressure_list)).T,np.vstack(df_[variable].values.flatten()),(X,Y),method='cubic').reshape(X.shape)
	Zm = np.ma.masked_where(np.isnan(Z),Z) 
	X,Y=np.meshgrid(df_.Date.values,interpolated_pressure_list) 
	plt.pcolormesh(X,Y,Zm,shading='gouraud',cmap=variable_list[variable][0],alpha=0.9) 
	cbar = plt.colorbar(label=variable_list[variable][1])

	CS = plt.contour(X,Y,Zm,linewidth=3,colors='k',alpha=.9)
...
def line_plot(df_):
	...
	plt.savefig(str(cruise)+'_'+variable+'_lineplot',format='eps')
	plt.close()
...
def ts_plot(df_):
	...
	plt.savefig(str(cruise)+'_tsplot',format='eps')	
	plt.close()
...
for cruise in df.Cruise.unique():
	...
	for variable in variable_list:
		print variable
		df_token = df[(df.Cruise==cruise)&(df.Pressure<1650)][['Pressure','Date','PosQC']+[variable]].dropna()
